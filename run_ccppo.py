...
from envs.flatland.observations.tree_obs import TreeObservation
import ray
...
load_models()
ray.init()
with open("ccppo_tree_obs/ccppo.yaml") as f:
    config = yaml.safe_load(f)
obs_config = config["flatland-random-sparse-small-tree-fc-cctransformer"]["config"]["env_config"]["observation_config"]
agent_config = config["flatland-random-sparse-small-tree-fc-cctransformer"]["config"]["model"]
...
#####################################################################
my_observation_builder = TreeObservation(obs_config)
...
# of the agents
#####################################################################
agent = CCPPOTrainer(config["flatland-random-sparse-small-tree-fc-cctransformer"]["config"], env=FlatlandSparse)
agent.restore("checkpoints/checkpoint_6084/checkpoint-6084")

def my_controller(obs, number_of_agents):
    _action = {}
    for _idx in range(number_of_agents):
        if _idx in obs.keys():
            _action[_idx] = agent.compute_action(observation=obs[_idx])
    return _action
...
while True:
    ...
    observation, info = remote_client.env_create(
                    obs_builder_object=my_observation_builder.builder()
                )
